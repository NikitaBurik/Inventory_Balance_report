...
invDF = pd.read_csv(invCSV[-1],  on_bad_lines='skip', dtype={'Value': np.float32})
grossDF = pd.read_csv(requirementCSV[-1], on_bad_lines='skip')
...
Inv_Bal_Cleaned = invDF[
    ['Indop', 'Plant', 'ANC', 'Description', 'Bal_Cat', 'Cat', 'BC', 'Supplier_ID', 'Supplier_Name', 'IDCO', 'CMDT',
     'STK_EUR', 'Value']]
Inv_Bal_Cleaned['Value'] = Inv_Bal_Cleaned['Value'].round(decimals=1)
...
weekHorizont = grossDF.iloc[:, 10:65]
...
df["INV F V"] = df["INV F V"].round(decimals=1)
df["INV H V"] = df["INV H V"].round(decimals=1)
...
def calculation_of_DIO_F_H_Total(df_combined):
    ...
    df_54_weeks = df_combined.iloc[:, 24:78]
    ...
    df_fh_q['DIO T'] = [x[df_fh_q.columns.get_loc('DIO T') + 1] if x[df_fh_q.columns.get_loc('Pos') + 1] > 0 else 0 for
                        x in df_fh_q.itertuples()]
    df_fh_q['DIO F'] = [0 if x[3] == 0 else (x[1] / x[3]) * x[58] for x in df_fh_q.itertuples()]
    df_fh_q['DIO H'] = [0 if x[3] == 0 else (x[2] / x[3]) * x[58] for x in df_fh_q.itertuples()]
    df_fh_q['MRP Q'] = [x[57] for x in df_fh_q.itertuples()]
    return df_fh_q
...
def merge_DIOs_with_TOTAL(df_dio, dfTOTAL):
    ...
    df_DIO_merged['MRP V'] = [x[12] * x[82] for x in df_DIO_merged.itertuples()]
...
# df_dio_clean.to_excel('test.xlsx')

# # # df_dio_clean[(df_dio_clean['MRP V']<1) & (df_dio_clean['MRP V']>0)]
# del combined_result.iloc[:, 18:21]
# combined_result = combined_result.drop(df.columns[[18:21]], axis=1)
# combined_result.to_excel('test.xlsx')
# dfs[dfs['DIO H']>0]
...
def Gross_Req_SS_Cases123(gross_req_turnover):
    ...
    gross_result_copy['#1'] = gross_result_copy['ANC'] + gross_result_copy['Plant']
